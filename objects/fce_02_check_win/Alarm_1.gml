...
if (!global.state.isFinal()) {
	instance_change(global.userIsAi[(global.playingAs[0] == global.state.player) ? 0 : 1] ? fce_01b_ai_move : fce_01a_player_move, true);
}
// Win: Notify and go to waiting state
	...
	if (playoutResult != 0) {
		if (global.userIsAi[(global.playingAs[0] == sign(playoutResult)) ? 0 : 1]) {
